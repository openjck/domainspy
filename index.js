...
let webtaskContext, webtaskCallback;
...
function getAPIServer() {
    ...
    if (!webtaskContext.secrets.SANDBOX || webtaskContext.secrets.SANDBOX !== 'true') {
...
function processDomains(apiServer, domainsToCheck) {
    ...
    for (const domains of domainGroups) {
        const namecheapAPIEndpoint = `${apiServer}/xml.response?ApiUser=${webtaskContext.secrets.NAMECHEAP_USERNAME}&ApiKey=${webtaskContext.secrets.NAMECHEAP_API_KEY}&UserName=${webtaskContext.secrets.NAMECHEAP_USERNAME}&Command=namecheap.domains.check&ClientIp=${ip.address()}&DomainList=${domains.join(',')}`;
        ...
        requestPromises.push(new Promise((resolve, reject) => {
            request(namecheapAPIEndpoint, (error, response, body) => {
                ...
                xml2js.parseString(body, (xmlParseError, { ApiResponse }) => {
                    if (xmlParseError) {
    ...
    Promise.all(requestPromises).then(() => {
        const missingDomains = domainsToCheck.filter(d => !domainsReturned.includes(d));
        if (missingDomains.length > 0) {
            throw missingDomains.map(md => `Could not get the status of ${md}`);
        }

        if (availableDomains.length === 0) {
            return webtaskCallback(null, 'SUCCESS: No domains are available');
        } else {
            let subject;
            ...
            return webtaskCallback(null, `SUCCESS: The following domains are available: ${availableDomains.join(', ')}`);
        }
    }).catch(err => {
        try {
            let errorMessage;
...
function email(rawSubject, messageOrMessages) {
    ...
    const validMsgArray = Array.isArray(messageOrMessages) &&
    ...
    sgMail.send({
        ...
        subject: `[${projectName}] ${rawSubject}`,
        text: body,
    });
}
